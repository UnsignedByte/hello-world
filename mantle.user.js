...
(function() {
  ...
  let b64asc = (b64pair) => (38 * base64chars.indexOf(b64pair[0]) + base64chars.indexOf(b64pair[1]));
  ...
  let base64chars = "b1cmt876i+gzxquas5pjof3ldhnre2k/w0yv94";
  ...
  class PortfolioStorer {
    ...
    setupPortfolio() {
      ...
      return Promise.all([
        fetchJSON(`users/${uid}/portfolios/${portfolioID}`, {'X-Csrf-Token': this.csrfToken}, 'PUT',
          {title: 'Mantle' + (this.desiredName ? ` (${this.desiredName})` : ''), description: 'Your ID: ' + this.id}),
    ...
    setContent(content) {
      content = encrypt(this.uid, content, true);
...
<input type="text" style="width: 100%; box-sizing: border-box; margin: 5px 0;" id="${UG_CSS_PFX}-auth-input" disabled>
    ...
    if (!ps) {
      ...
      if (!userID) {
        ...
        await ps.setContent(stringify({name: desiredName, bio: 'I love SELF! All hail the administration!', following: ['1827123-2779284-7167500-9040b7b9cfd642bf097413615a1cfcf3-mantle'], posts: [], comments: [], likes: []}));
      }
      window.ps = ps;
    }
    await refresh();
